...
\begin{exem}\label{exe:ComputacaoAFD1}
	Considere o AFD do Exemplo \ref{exe:AFDA} e a palavra de entrada $aaaa$ tem-se que a computação desta palavra corresponde a:
	\begin{eqnarray*}
...
\begin{exem}\label{exe:AceiteAFD1}
	Considerando os Exemplos \ref{exe:ComputacaoAFD1} e \ref{exe:ComputacaoAFD2} tem-se que a palavra $aaaa$ é aceita pelo AFD do Exemplo \ref{exe:ComputacaoAFD1}, uma vez que, $q_0 \in F$. Por outro, lado a palavra $0101$ não é aceita pelo AFD do Exemplo \ref{exe:ComputacaoAFD2}, uma vez que, $s_1 \notin F$, de fato o leitor atento pode notar que o AFD do Exemplo \ref{exe:ComputacaoAFD2} não aceita qualquer palavra de entrada pois $F = \emptyset$.
\end{exem}
...
Utilizando a definição acima o leitor deve ser capaz de perceber que se um AFD reconhece uma linguagem $L \subseteq \Sigma^*$, então ele para em estados finais apenas para as palavras $w \in L$. Em outra palavra para mostrar que uma linguagem $L$ é a linguagem de um AFD $A$, deve-se provar que $L = \mathcal{L}(A)$.

\begin{exem}\label{exe:AFDLinguagem1}
	O autômatos $A_1$ representado pelo diagrama abaixo,
	\begin{figure}[h]
		\centering
		\begin{tikzpicture}[>=stealth, shorten >=1pt, node distance=2.0cm, on grid, auto, state/.append style={minimum size=3em}, thick ]
			\node[state, initial,accepting]		(A)               {$q_0$};
			\node[] 							(B) [right of=A]  {};
			\node[state]						(C) [above of=B]  {$q_1$};
			\node[state, accepting]				(D) [below of=B]  {$q_2$};
			\node[state] 						(E) [right of=B]  {$q_3$};
			...
			(C) edge 			  				node {$b$}     		 (D)
			(C) edge 			  				node {$a$}     		 (E)
			(D) edge			  				node [right]{$a,b$}	 (E)
			(E) edge [loop right]				node {$a,b$} 		 ( );
		\end{tikzpicture}
		\caption{AFD $A_1$.}
		\label{fig:AFDLinguagem1}
	\end{figure}
	
	\noindent reconhece a linguagem $L = \{\lambda, ab, b\}$. Para verificar a informações anterior primeiro note que,	
	\begin{eqnarray*}
		\widehat{\delta}(q_0, \lambda) = \delta(q_0, \lambda) = q_0
	\end{eqnarray*}
	e também, 
	\begin{eqnarray*}
		\widehat{\delta}(q_0, b) = \delta(\widehat{\delta}(q_0, \lambda), b) = \delta(q_0, b) = q_2
	\end{eqnarray*}
	além disso, 	
	\begin{eqnarray*}
		\widehat{\delta}(q_0, ab) = \delta(\widehat{\delta}(q_0, a), b) = \delta(\delta(\widehat{\delta}(q_0,\lambda), a), b) = \delta(\delta(q_0, a), b) = \delta(q_1, b) = q_2
	\end{eqnarray*}
	como $q_0, q_2 \in F$ tem-se que para todo $w \in L$ a relação $w \in \mathcal{L}(A)$ é verdadeira, consequentemente, $L \subseteq \mathcal{L}(A)$. Agora suponha por absurdo que existe $w \in \Sigma^*-\{\lambda, b, ab\}$ tal que $w \in \mathcal{L}(A)$, entretanto, para o caso que $|w| \geq 3$ pela definição de $\delta$ é claro que $\delta(q_0, w) = q_3$ (a prova disso fica como exercício ao leitor), e como $q_3 \notin F$ tem-se que $w \notin \mathcal{L}(A)$ o que contradiz a hipótese inicial, e portanto, pode-se concluir disso que, 
	\begin{equation}\label{eq:ExemploLinguagemAFD1}
		\mathcal{L}(A) \subseteq \Sigma^* - \Big(\bigcup_{i = 3}^\infty \Sigma^i\Big)
	\end{equation}
	por outro lado, para o caso que $|w| < 3$ tem-se obrigatoriamente que $w \in \Sigma^2 - \{ab\}$ ou $w \in \Sigma - \{b\}$, para ambos os casos desenvolvendo $\widehat{\delta}(q_0, w)$ tem-se que  $\widehat{\delta}(q_0, w) \notin F$, o que contradiz a hipótese inicial, e portanto, pode-se concluir disso que, 
	\begin{equation}\label{eq:ExemploLinguagemAFD2}
		\mathcal{L}(A) \subseteq \Sigma^* - \Big(\Big(\Sigma \cup \Sigma^2\Big) - \{b, ab\}\Big)
	\end{equation}
	consequentemente, das Equações $(\ref{eq:ExemploLinguagemAFD1})$ e $(\ref{eq:ExemploLinguagemAFD2})$ segue que, 
	\begin{equation}\label{eq:ExemploLinguagemAFD3}
		\mathcal{L}(A) \subseteq \Sigma^* - \Big(\Big(\bigcup_{i = 1}^\infty \Sigma^i\Big) - \{b, ab\}\Big)
	\end{equation}
	agora pela definição de $\delta$ é fácil notar que para todo $w \in \{\lambda, ab, b\}$ tem-se que $\widehat{\delta}(q_0, w) \in F$, desse fato e da Equação $(\ref{eq:ExemploLinguagemAFD3})$ tem-se que,  
	$$\mathcal{L}(A) \subseteq \Sigma^* - \Big(\Big(\bigcup_{i = 0}^\infty \Sigma^i\Big) - \{\lambda, b, ab\}\Big) = \{\lambda, b, ab\}$$
	e portanto, todo $w \in \mathcal{L}(A)$ é tal que $w \in L$, assim por definição tem-se que $\mathcal{L}(A) \subseteq L$. Por fim, uma vez que, $L \subseteq \mathcal{L}(A)$ e  $\mathcal{L}(A) \subseteq L$ por definição tem-se que $L = \mathcal{L}(A)$, o que completa a verificação.
\end{exem}

\begin{exem}\label{exe:AFDLinguagem2}
	Seja $L = \{bba^{2n} \mid n \in \mathbb{N}\}$ é uma linguagem reconhecida pelo AFD a seguir
\end{exem}
