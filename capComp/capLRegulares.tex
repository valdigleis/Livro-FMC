...
\begin{definition}[Transição não-determinística estendida]\label{def:FuncaoDeltaNDEstendida}
	...
	\begin{eqnarray}\label{eq:FuncaoDeltaNDEstendida}
		...
		\widehat{\delta_N}(q, wa)& = & \bigcup_{q' \in \widehat{\delta_N}(q, w)} \delta_N(q', q)
	\end{eqnarray}
\end{definition}
...
\begin{exem}\label{exe:ComputacaoAFN}
	...
	\begin{eqnarray}\label{eq:AFNComptuacao3}
		...
		& = & \{q_1\} \nonumber
	\end{eqnarray}
	substituindo a Equação (\ref{eq:AFNComptuacao3}) ma Equação (\ref{eq:AFNComptuacao2}) tem-se que, 
	\begin{eqnarray}\label{eq:AFNComptuacao4}
		\widehat{\delta_N}(q_0, ab) & = & \{q_0, q_2\}
	\end{eqnarray}
	e finalmente substituindo a Equação (\ref{eq:AFNComptuacao4}) ma Equação (\ref{eq:AFNComptuacao1}) tem-se que,
	 \begin{eqnarray}
...
Pelo exemplo anterior o leitor pode ter notado que diferente do caso determinístico, a computação em um AFN não é linear, no sentido de um único caminho de computação\footnote{Como explicado em \cite{valdi2020phd} um caminho de computação é uma sequência de estados assumidos pela unidade central do autômato durante o processamento de uma palavra de entrada.}, em vez disso, a computação em um AFN pode ser vista como uma árvore em que a união dos estados em cada nível da árvore representa a superposição de estados assumida pela unidade de controle do autômato a cada símbolo consumido da palavra $w$, o exemplo a seguir ilustra essa ideia.

\section{Questionário}

\begin{rema}
	Quando $w \in \Sigma^*$ e $c \in \Sigma$ a notação $|w|_c$ representa o número de $c$'s que existem em $w$.
\end{rema}
