...
\begin{exem}\label{exe:AceitacaoEmAFN}
	Considerando o AFN representado pela Figura \ref{fig:AFN2} a seguir tem-se que $\widehat{\delta_N}(q_0, aabbbba) = \{q_1, q_3\}$ logo ``$aabbbba$'' é aceita por tal AFN, pois uma vez que, $q_1 \in F$ segue que  $\widehat{\delta_N}(q_0, aabbbba) \cap F \neq \emptyset$. Por outro lado, para a palavra ``$aabb$'' tem-se que $\widehat{\delta_N}(q_0, aabb) = \{q_2, q_3\}$, consequentemente, $\widehat{\delta_N}(q_0, aabb) \cap F = \emptyset$, e portanto, ``$aabb$'' não é aceita pelo AFN.
