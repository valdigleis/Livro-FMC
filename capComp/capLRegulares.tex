...
\begin{algorithm}[h]
	...
	\caption{Algoritmo para converter AFN em AFD sem estados inúteis.}
	\label{alg:AFN-AFD}
\end{algorithm}
...
\begin{exem}
	...
	\begin{eqnarray}\label{eq:ExemLAFN2}
		...
		& = & \bigcup_{q' \in \{q_2\}} \widehat{\delta_\lambda}(\underline{\delta_N}(q', 1))\\
		& = & \widehat{\delta_\lambda}(\underline{\delta_N}(q_2, 1))\nonumber\\
		& = & \{q_0, q_1, q_2\}\nonumber
	\end{eqnarray}
	substituindo o valor da Equação (\ref{eq:ExemLAFN2}) na Equação (\ref{eq:ExemLAFN2}) tem-se que, 
	\begin{eqnarray*}
		\widehat{\underline{\delta_N}}(q_0, 10) & = & \bigcup_{q' \in \{q_0, q_1, q_2\}} \widehat{\delta_\lambda}(\underline{\delta_N}(q', 0))\\
		& = & \widehat{\delta_\lambda}(\underline{\delta_N}(q_0, 0)) \cup \widehat{\delta_\lambda}(\underline{\delta_N}(q_1, 0)) \cup \widehat{\delta_\lambda}(\underline{\delta_N}(q_2, 0))\\
		& = & \{q_0, q_1, q_2\}
	\end{eqnarray*}
\end{exem}
...
Os aspectos relacionados a mostrar que um $\lambda$-AFN reconhece uma linguagem $L$ são similares ao mesmo aspectos com respeito aos AFN. A seguir serão mostrados alguns $\lambda$-AFN e as linguagens aceitas por eles, ficando a prova dessas aceitações como atividade de exercício ao leitor. 
...
\begin{rema}
	A partir deste ponto sempre que $w \in \Sigma^*$ e $c \in \Sigma$ a notação $|w|_c$ irá representar o número de $c$'s que existem na palavra $w$.
\end{rema}

\begin{exem}\label{exe:LinguagemLAFN2}
	O $\lambda$-AFN representado pelo grafo de transição esboçado pela Figura \ref{fig:Linguagem-LAFN2}  a seguir aceita a linguagem $L = \{uv \in \{a,b\}^* \mid u \in \{a\}^*, |u|_a = 2k\text{ ou } |u|_a = 3k, v=bx, x \in \{a,b\}^*, k \in \mathbb{N}\}$.
	...
	\begin{figure}[h]
		...
		\begin{tikzpicture}[>=stealth, shorten >=1pt, node distance=3.0cm, on grid, auto, state/.append style={minimum size=3em}, thick ]
			...
			\node[state]						(E) [right of=B] 	{$s_3$};
			\node[state, accepting]				(F) [right of=E] 	{$s_4$};
			\node[state]				        (G) [left of=D] 	{$s_5$};
			\path[->] (A) +(-1,0) edge (A)
			
			%Transições:
			%(Partida) edge [tipo da seta] node {simbolo lido} (Destino)
			(A) edge 			  				node [below] {$\lambda$} (E)
...
Como discutido em \cite{benja-2011, benja-2015, hopcroft2008, linz2006}, as $\lambda$-transições não aumentam o poder computacional dos autômatos finitos, assim o problema de remover as $\lambda$-transições dos $\lambda$-AFN deve ser decidível, de fato, como mostrado em \cite{benjaLivro2010, hopcroft2008} existe um algoritmo que é capaz de remover todas as $\lambda$-transições de qualquer um $\lambda$-AFN, a seguir tal algoritmo é apresentado.
...
Argumentações sobre a corretude do Algoritmo \ref{alg:LAFN-AFD} podem ser consultadas em \cite{hopcroft2008}. Um ponto interessante de tal algoritmo é que ele não apenas remove as $\lambda$-transições como também elimina simultaneamente o não-determinismo por símbolo e ainda preservar a linguagem reconhecida como explicado em \cite{benjaLivro2010}. 
